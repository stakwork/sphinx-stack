...
RNCryptor.Encrypt = function (password, plaintext, options) {
  options = options || {};
  var encryption_salt =
    options["encryption_salt"] || sjcl.random.randomWords(8 / 4); // FIXME: Need to seed PRNG
...
RNCryptor.Decrypt = function (password, message, options) {
  ...
  var expected_hmac = new sjcl.misc.hmac(hmac_key).encrypt(
    sjcl.bitArray.bitSlice(message, 0, ciphertext_end)
  );
...
var JSCryptor = {};
// returns base64
JSCryptor.Encrypt = function (plaintext_in, password) {
  const plaintext = sjcl.codec.utf8String.toBits(plaintext_in);
  var enc = RNCryptor.Encrypt(password, plaintext);
